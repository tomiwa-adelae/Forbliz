...
import { forblizNavLinks } from "@/constants";
...
export const Header = () => {
  ...
  return (
    <header className="fixed top-0 z-50 w-full bg-[#040d0a] overflow-hidden border-b border-white/5">
      ...
      <div className="container h-20 flex items-center justify-between relative z-10">
        ...
        {/* Navigation */}
        <nav className="hidden lg:flex items-center text-white gap-2 font-semibold text-sm">
          {forblizNavLinks.map(({ slug, label }, index) => (
            <Button
              size={"md"}
              key={index}
              asChild
